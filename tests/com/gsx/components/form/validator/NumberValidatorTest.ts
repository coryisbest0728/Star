...
describe('The unit test for the NumberValidator', function () {
    ...
    it('Async validation of the number validator', function (done) {
        ...
        Promise.all([
            ...
            expect(isValids[6]).not.toBeTruthy(validatedValue3 + ' is not a number.');
            expect(isValids[7]).not.toBeTruthy(validatedValue4 + ' is not a number.');
            expect(isValids[8]).not.toBeTruthy(validatedValue5 + ' is not a number.');
            expect(isValids[9]).not.toBeTruthy(validatedValue6 + ' is not a number.');
            expect(isValids[10]).not.toBeTruthy(validatedValue6 + ' is not a number.');
            done();
        });
    });
});
